...
const TopNavbar: React.FC<TopNavbarProps> = ({ sidebarOpen, setSidebarOpen }) => {
    ...
    useEffect(() => {
        const fetchPendingBookings = async () => {
            try {
                ...
                setNotificationCount(response.data.ptotalCount); // Get the count of pending bookings
                // Get the full names from pending bookings
    ...
    return (
        <>
            ...
            {/* Top Navbar */}
            <div className="bg-gradient-to-r from-green-400 to-blue-500 text-white p-4 flex justify-between items-center fixed top-0 left-60 right-0 z-10 shadow-lg">
                <button className="md:hidden text-white" onClick={() => setSidebarOpen(!sidebarOpen)}>
                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16"></path>
                    </svg>
                </button>
                <div className="text-2xl font-semibold tracking-wide ml-4">3DRealSpace Scans</div>

                {/* Notification Bell */}
                <div className="relative ml-[45rem]" ref={dropdownRef}> {/* Increased margin-left to move further right */}
                    <button
                    ...
                    {notificationDropdownOpen && (
                        <div className="absolute right-0 mt-2 w-[25rem] bg-white rounded-lg shadow-lg overflow-hidden z-20">
                            {notifications.length > 0 ? (
                                notifications.map((name, index) => (
                                  <div key={index} className="px-4 py-2 text-gray-500 text-sm font-light hover:bg-gray-100 transition-colors duration-150 cursor-pointer" onClick={() => navigate('/view-booking')}>
                                      You got a new booking enquiry from {name}
                                  </div>
                              
                                ))
                            ) : (
                                <div className="px-4 py-2 text-gray-500 text-sm font-light">No new enquiries</div>
                            )}
                        </div>
                    )}
                </div>
                ...
                <div className="relative">
                    <button
                        className="flex items-center space-x-2 hover:bg-opacity-50 px-4 py-2 rounded-lg focus:outline-none"
